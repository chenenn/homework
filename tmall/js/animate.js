...
Float.prototype.resizes = function() {
  this.maxX = window.innerWidth - this.obj.offsetWidth;
  this.maxY = window.innerHeight - this.obj.offsetHeight;
}
...
Float.prototype.move = function() {
  ...
  this.t = setInterval(function() {
    ...
    if(lefts>that.maxX) {
      that.speed *= -1;
      that.obj.style.background = 'pink';
    }
    if(lefts<0) {
      that.speed *= -1;
      that.obj.style.background = 'grey';
    }
    if(tops>that.maxY) {
      that.speed1 *= -1;
      that.obj.style.background = 'green';
    }
    if(tops<0) {
      that.speed1 *= -1;
      that.obj.style.background = 'red';
    }
    that.obj.style.left = (lefts+that.speed) + 'px';
    that.obj.style.top = (tops+that.speed1) + 'px';
  } , 100)
}